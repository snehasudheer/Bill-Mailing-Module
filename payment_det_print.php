	...
	if(isset($_SESSION['bill_type']) && $_SESSION['bill_type']!='')
		{
			...
			if(trim($bill_type)=='Monthly')
			{
				...
				if($cat=='LT4')
				{
					$stid = oci_parse($conn, "SELECT MONTH as term,YEAR as year,BILLNO as bill_no,BILLDATE as bill_date,PAMT as pamt,PAYDATE as pdate,RECEIPTNO as receipt FROM LT4billfile where CNO='$consumerno' and YEAR = '$year' order by YEAR,MONTH");
				}
				else if($cat=='GT')
				{
					$stid = oci_parse($conn, "SELECT MONTH as term,YEAR as year,BILLNO as bill_no,BILLDATE as bill_date,PAMT as pamt,PAYDATE as pdate,RECEIPTNO as receipt FROM GTbillfile where CNO='$consumerno' and YEAR = '$year' order by YEAR,MONTH");		
				}
				else if($cat=='HT')
				{ 
					$stid = oci_parse($conn, "SELECT MONTH as term,YEAR as year,BILLNO as bill_no,BILLDATE as bill_date,PAMT as pamt,PAYDATE as pdate,RECEIPTNO as receipt FROM NEWHTbillfile where CNO='$consumerno' and YEAR = '$year' order by YEAR,MONTH");
				}
			}
			elseif(trim($bill_type)=='Spot')
				{
					...
					$year = $year.'-'.$yr;
					$stid = oci_parse($conn, "SELECT TERM,YEAR,BNO as bill_no,BDATE as bill_date,PAMT,PDATE,RECEIPT FROM billfile_trans where CNO='$consumerno' and YEAR = '$year' order by BDATE DESC");
					//echo "SELECT TERM,YEAR,BNO as bill_no,BDATE as bill_date,PAMT,PDATE,RECEIPT FROM billfile_trans where CNO='$consumerno' and YEAR = '$year' order by BDATE DESC";
					//exit;
				}
		}
...
<tr>
	...
	<th align="right" style="border-bottom-width:0.1px; width:20%;">&nbsp;</th>
		...
		<h2 align="center">Payment Details '.$year.'</h2>
	 ...
	 <th class="col-md-2" style="text-align:center;"><b>Paid Amount</b></th>
	 <th class="col-md-2" style="text-align:center;"><b>Paid Date</b></th>
	 <th class="col-md-2" style="text-align:center;"><b>Receipt No.</b></th>
	 </tr>
	 <tbody>';
	  $count=0;
	 $cur_date = date('d-M-y');
...
$pdf->Output('payment_details'.$year.'.pdf', 'D');
}
$year = date("Y"); 
...
<div class="container" style="margin-top:2%;">
	 ...
	 <form name="bill_det" method="post" action="payment_det_print.php">
